...
- **Workflow**: `execution_count` and `failure_count` no longer cause "Provider produced inconsistent result after apply" errors when updating a workflow that has been executed at least once. These fields are server-side live metrics — SailPoint resets them to 0 on every PUT — so the `UseStateForUnknown` plan modifier was removed and the values are now refreshed unconditionally on every Read.
...
- **Form Definition**: Fix "inconsistent result after apply" error caused by `omitempty` dropping empty `validations` arrays and `key` strings during JSON round-trip of `form_elements`
- **Form Definition**: Add missing `Config` field to `FormElementValidation` to preserve validation config objects (e.g., `MIN_LENGTH` with min value)

## [2.3.1] - 2026-03-15

### Fixed

- **Source**: Change `features` from List to Set to prevent spurious plan diffs caused by API returning features in different order
...
- **Identity Attribute**: Fixed "Provider produced inconsistent result after apply" error for `properties` field in sources
  - Applied `jsontypes.Normalized` to the `properties` field for semantic JSON comparison
  - Prevents state drift when API normalizes JSON casing (e.g., `ruleType: "identityAttribute"` → `"IdentityAttribute"`)
...
[0.5.1]: https://github.com/AnasSahel/terraform-provider-sailpoint-isc-community/compare/v0.5.0...v0.5.1
[0.5.0]: https://github.com/AnasSahel/terraform-provider-sailpoint-isc-community/compare/v0.3.0...v0.5.0
