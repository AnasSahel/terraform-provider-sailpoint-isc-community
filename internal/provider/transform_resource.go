...
func (r *transformResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			fmt.Sprintf("Could not create transform, invalid attributes_json: %s", err.Error()),
		)
		return
	}

	// transform := api_v2025.NewTransformWithDefaults()
	// transform.SetType(plan.Type.ValueString())
	// transform.SetName(plan.Name.ValueString())
	// transform.SetAttributes(attributesMap)
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Error Creating Transform",
			fmt.Sprintf("Could not create transform, unexpected error: %s\n, Full Response: %v", err.Error(), response),
		)
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Error Creating Transform",
			fmt.Sprintf("Could not create transform, unexpected error: %s", err.Error()),
		)
		return
	}
...
func (r *transformResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			fmt.Sprintf("Could not read transform, unexpected error: %s\n, Full Response: %v", err.Error(), response),
		)
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Error Reading Transform",
			fmt.Sprintf("Could not read transform, unexpected error: %s", err.Error()),
		)
		return
	}
